...
TRAP_PUTS
	...
	LC R1, OS_ADSR_ADDR ;load status register of ascii display in r1
	LDR R1, R1, #0 ;read value that was in status register
	BRzp TRAP_PUTS ; if negative continue otherwise go back until status is negative

	LC R1, OS_ADDR_ADDR ; load address of data register
	LDR R3, R0, #0 ; load value stored in r0 address

	CMPIU R3, #0 ; 0x30 is hex code for 0 character in ASCII

	;;; if the value in r0 is not a null character then continue

	BRz END ; if r0 contains a null character then end

	STR R3, R1, #0 ; write character to ascii data display register

	ADD R0, R0, #1  ; how do you set R0s data address to the new incremented value
...
;; Want to call TRAP_PUTC
	
;;;  YOUR CODE HERE
;;;  Hint: Don't forget to check the status register everytime you need to
